...
app.config['Upload_FOLDEER'] = './static/profile_pics'
...
def sign_in():
    # Sign in or # an api endpoint for logging in
    ...
    # password combination cannot be found
        ...
        return jsonify(
            {
                "result": "fail",
                "msg": "Opps, sorry! ID or Password incorrect!",
            }
        )
...
def update_profile():
    ...
    try:
        ...
        if 'file_give' in request.files:
            ...
            file_path = f'profile_pics/{username}.{extention}'
...
def get_posts():
    ...
    try:
        ...
        for post in posts:
            ...
            post['count_bookmark'] = db.likes.count_documents({
                'post_id': post['_id'],
                'type': 'bookmark',
            })
            ...
            post['heart_by-me'] = bool(db.likes.find_one({
                'post_id': post['_id'],
                'type': 'heart',
                'username': payload.get('id')
            }))

            post['bookmark_by-me'] = bool(db.likes.find_one({
                'post_id': post['_id'],
                'type': 'bookmark',
                'username': payload.get('id')
            }))

            post['thumbsup_by-me'] = bool(db.likes.find_one({
                'post_id': post['_id'],
                'type': 'thumbsup',
                'username': payload.get('id')
            }))
